...
interface GameState {
  ...
  gameMode: GameMode;
  eatOrder: number[]; // shuffled indices for 2D mode; empty array for 1D mode
}
...
function buildInitialState(
  ...
  let foodPositions: Map<string, number>;
  if (gameMode === "2D") {
    // In 2D mode, place ALL letters like 1D mode but with a shuffled eat order
    const snakeStart: Position[] = [
      { x: Math.min(cx + 3, gridSize - 1), y: cy },
      { x: Math.min(cx + 2, gridSize - 1), y: cy },
      { x: Math.min(cx + 1, gridSize - 1), y: cy },
    ];
    const foodPositions2D = placeAllLetters(
      snakeStart,
      LETTER_SETS[setIndex].letters.length,
      gridSize,
    );
    const eatOrder = shuffle(
      Array.from({ length: LETTER_SETS[setIndex].letters.length }, (_, i) => i),
    );
    return {
      snake: snakeStart,
      foodPositions: foodPositions2D,
      direction: "LEFT",
      status: "idle",
      score,
      level,
...
export default function SnakeGame() {
  ...
  const [displayState, setDisplayState] = useState<GameState>(() =>
    buildInitialState(0, 0, 1, 750, 12, "1D"),
  );
  ...
  tickRef.current = () => {
    ...
    const currentTargetLetterIdx =
      s.gameMode === "2D"
        ? (s.eatOrder?.[s.targetLetterIndex] ?? -1)
        : s.targetLetterIndex;
    const isCorrectFood = isAnyFood && foodLetterIdx === currentTargetLetterIdx;

    // Both 1D and 2D modes: wrong letter = game over
    ...
    const eatenLetter = currentSet.letters[currentTargetLetterIdx] ?? "";
    ...
    if (newTargetIndex >= currentSet.letters.length) {
      ...
      if (status === "levelcomplete") {
        ...
        levelTimerRef.current = setTimeout(() => {
          ...
          nextDirRef.current = s.gameMode === "2D" ? "LEFT" : "RIGHT";
        }, 2200);
      }
      return;
    }

    const updatedFoodPositions = newFoodPositions;
  ...
  // Audio loop for 2D mode — speaks the current letter every 2500ms
  useEffect(() => {
    const s = stateRef.current;

    // Clear any existing audio loop first
    if (audioLoopRef.current) {
      clearInterval(audioLoopRef.current);
      audioLoopRef.current = null;
    }

    if (displayState.gameMode === "2D" && displayState.status === "playing") {
      const currentSet = LETTER_SETS[s.setIndex];
      const currentIdx =
        displayState.gameMode === "2D"
          ? (displayState.eatOrder?.[displayState.targetLetterIndex] ??
            displayState.targetLetterIndex)
          : displayState.targetLetterIndex;
      const letter = currentSet.letters[currentIdx] ?? "";

      // Speak immediately first
      speakLetter(letter, currentSet.isEnglish, s.setIndex);

      // Then repeat every 2500ms
      audioLoopRef.current = setInterval(() => {
        const cur = stateRef.current;
        if (cur.status !== "playing" || cur.gameMode !== "2D") {
          if (audioLoopRef.current) {
            clearInterval(audioLoopRef.current);
            audioLoopRef.current = null;
          }
        ...
        const curSet = LETTER_SETS[cur.setIndex];
        const curTargetIdx =
          cur.gameMode === "2D"
            ? (cur.eatOrder?.[cur.targetLetterIndex] ?? cur.targetLetterIndex)
            : cur.targetLetterIndex;
        const curLetter = curSet.letters[curTargetIdx] ?? "";
        speakLetter(curLetter, curSet.isEnglish, cur.setIndex);
      }, 2500);
    ...
    // If not 2D playing, ensure audio is cancelled
    if (window.speechSynthesis) window.speechSynthesis.cancel();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    displayState.gameMode,
    displayState.status,
    displayState.targetLetterIndex,
    displayState.eatOrder,
  ]);
  ...
  const handleNewGame = useCallback(
    (setIndex?: number, mode?: GameMode) => {
      const idx = setIndex !== undefined ? setIndex : stateRef.current.setIndex;
      const gameMode = mode !== undefined ? mode : stateRef.current.gameMode;
      if (levelTimerRef.current) clearTimeout(levelTimerRef.current);
      nextDirRef.current = gameMode === "2D" ? "LEFT" : "RIGHT";
      ...
      syncState(
        buildInitialState(
          ...
          0,
          1,
          stateRef.current.speedInterval,
          gridSizeRef.current,
          gameMode,
        ),
      );
  ...
  const handleReplay = useCallback(() => {
    ...
    const targetIdx =
      s.gameMode === "2D"
        ? (s.eatOrder?.[s.targetLetterIndex] ?? s.targetLetterIndex)
        : s.targetLetterIndex;
    speakLetter(set.letters[targetIdx] ?? "", set.isEnglish, s.setIndex);
  }, []);

  const handleModeSwitch = useCallback(
    (newMode: GameMode) => {
      const s = stateRef.current;
      if (s.gameMode === newMode) return;
      if (levelTimerRef.current) clearTimeout(levelTimerRef.current);
      nextDirRef.current = newMode === "2D" ? "LEFT" : "RIGHT";
      setGameOverReason(null);
      syncState(
        buildInitialState(
          s.setIndex,
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .center-indicator {
          box-shadow: inset 0 0 0 2px rgba(255, 200, 50, 0.25);
        }
      ...
      <div className="snake-root" data-ocid="game.page">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span
              style={{
                ...
                fontWeight: 700,
                padding: "2px 8px",
                background: s.gameMode === "2D" ? "#7c3aed" : "#065f46",
                color: s.gameMode === "2D" ? "#ddd6fe" : "#6ee7b7",
                borderRadius: 20,
                fontFamily: tamilFont,
                border: `1px solid ${s.gameMode === "2D" ? "rgba(167,139,250,0.4)" : "rgba(52,211,153,0.3)"}`,
              }}
            >
              {s.gameMode === "2D" ? "2ஆம் பரிமாணம்" : "1ஆம் பரிமாணம்"}
            </span>
          </div>
        ...
        <main className="snake-main">
          ...
          <div className="snake-infobar">
            ...
            {/* In 2D mode, show the current letter prominently */}
            {s.gameMode === "2D" && (
              <div
                style={{
                  display: "flex",
              ...
              >
                ...
                >
                  {currentSet.letters[
                    s.gameMode === "2D"
                      ? (s.eatOrder?.[s.targetLetterIndex] ?? 0)
                      : s.targetLetterIndex
                  ] ?? ""}
                </span>
              </div>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  <button
                    ...
                    style={{
                      flex: 1,
                      background: s.gameMode === "1D" ? "#16a34a" : "#0e2d50",
                      color: s.gameMode === "1D" ? "#fff" : "#94a3b8",
                      border:
                        s.gameMode === "1D"
                          ? "1px solid #22c55e"
                  ...
                  <button
                    ...
                    style={{
                      flex: 1,
                      background: s.gameMode === "2D" ? "#7c3aed" : "#0e2d50",
                      color: s.gameMode === "2D" ? "#fff" : "#94a3b8",
                      border:
                        s.gameMode === "2D"
                          ? "1px solid #a78bfa"
                          : "1px solid transparent",
